...
from eoms.model.promotion import get_all_current_promo, get_products_by_promo_code
from eoms.route import inventory

# Promotion list
@app.route('/promotion')
def view_promotion():
    promotion_list = get_all_current_promo()
...
def view_promotion_products(promo_code):
    ...
    if promo and promo.get('status') == 1 and promo.get('end_date') >= datetime.now():
        equipment_list = get_products_by_promo_code(promo_code)
        current_category = {'name': promo['name']} if  promo['name'] else {'name': promo['promo_code']} 
